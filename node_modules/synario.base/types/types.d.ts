...
export type Mime = {
    [key in TransmissionTypee]?: string;
};
...
export type SubInt<T extends number, _Arr extends number[] = []> = T extends number ? _Arr['length'] extends T ? _Arr[number] : SubInt<T, [..._Arr, _Arr['length']]> : never;
export type UnionToIntersection<T> = (T extends T ? (params: T) => any : never) extends (params: infer P) => any ? P : never;
export type UnionToTuple<T, Res extends any[] = []> = UnionToIntersection<T extends any ? () => T : never> extends () => infer ReturnType ? UnionToTuple<Exclude<T, ReturnType>, [...Res, ReturnType]> : Res;
...
export type FlattenObjectKey<T> = T extends any[] ? T : T extends object ? UnionToIntersection<Obj<T[keyof T]>> extends object ? NonObjectPicks<T> & FlattenObjectKey<ObjectPicks<T>[keyof ObjectPicks<T>]> : T : never;
